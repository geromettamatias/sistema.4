...
$consulta = "SELECT `id_mensaje`, `id_usuario`, `fecha`, `mensaje`, `datos` FROM `mensajes_admin` WHERE `id_usuario`='$idDocente'";
$resultado = $conexion->prepare($consulta);
$resultado->execute();
$data=$resultado->fetchAll(PDO::FETCH_ASSOC);
...
?>
    ...
    <input type="text" class="form-control" id="idAlumnoCos" value="<?php echo $idDocente ?>" hidden="">
...
function  eliminarmensajeAdminFina(id_mensaje,opcion){
    ...
    if(respuesta){
        ...
        $.ajax({
            url: "bd/crud_mensajeAdminProfesor.php",
            type: "POST",
            dataType: "json",
...
$("#formPersonasMensajeAdmin").submit(function(e){
    ...
    console.log({id_mensaje:id_mensaje, id_usuario:id_usuario, fecha:fecha, mensaje:mensaje, datos:datos, opcion:opcion});
    ...
    $.ajax({
        ...
        type: "POST",
        dataType: "json",
        data: {id_mensaje:id_mensaje, id_usuario:id_usuario, fecha:fecha, mensaje:mensaje, datos:datos, opcion:opcion},
        success: function(data){  
            ...
            if(opcion == 1){tablaAlumnoMensaje.row.add([id_mensaje,id_usuario,fecha,mensaje,datos]).draw();}
            else{tablaAlumnoMensaje.row(fila).data([id_mensaje,id_usuario,fecha,mensaje,datos]).draw();} 

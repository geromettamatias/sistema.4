...
$consulta = "SELECT `id_mensaje`, `id_usuario`, `fecha`, `mensaje`, `datos` FROM `mensajes` WHERE `id_usuario`='$idAlumnos'";
$resultado = $conexion->prepare($consulta);
$resultado->execute();
$data=$resultado->fetchAll(PDO::FETCH_ASSOC);
...
function  eliminarmensajeAdminFina(id_mensaje,opcion){
    ...
    if(respuesta){
        ...
        $.ajax({
            url: "bd/crud_datos_mensajeAdmin.php",
            type: "POST",
            dataType: "json",
...
$("#formPersonasMensajeAdmin").submit(function(e){
    ...
    $.ajax({
        ...
        success: function(data){  
            ...
            id_mensaje1 = data[0].id_mensaje;            

            fecha1 = data[0].fecha;
            mensaje1 = data[0].mensaje;
            datos1 = data[0].datos;

            if(opcion == 1){tablaAlumnoMensaje.row.add([id_mensaje1,fecha1,mensaje1,datos1]).draw();}
            else{tablaAlumnoMensaje.row(fila).data([id_mensaje1,fecha1,mensaje1,datos1]).draw();} 
